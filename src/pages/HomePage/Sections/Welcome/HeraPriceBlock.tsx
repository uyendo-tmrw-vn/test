...
export default function HeraPriceBlock({ }: Props): ReactElement {
  const heraValue = useFetchHeraVal();
  const cmsData = useSelector((state) => state.cmsSlice.cmsData);

  return (
    <>
      <div className="d-flex flex-column justify-content-between align-items-center heraprice-block">
        <div className="hera-price mb-1">
          HERA Price - ${heraValue.heraPrice}
        </div>
        <div className="w-100 px-2 d-flex justify-content-between align-items-center py-2">
          {cmsData?.data?.attributes?.exchangeInformation?.map((obj, index) => (
            <div className="d-flex align-items-center provider-block" key={index}>
          ...
          {/* <div className="d-flex align-items-center provider-block border-r">
            <div className="provider d-flex flex-column justify-content-center align-items-center">
              <img src={GateIO} className="icon" alt="" />
              <div>Gate.io</div>
            </div>
            <a
              href="https://www.gate.io/trade/HERA_USDT"
              target="_blank"
              rel="noreferrer"
            >
              <div className="ml-2 buy-hera">Buy HERA</div>
